...
class Producer:
    def __init__(self, bootstrap_servers):
        ...
        try:
            ...
            logger.error("kafka producer - Exception during connecting to broker - {}".format(exc))
            return False
...
class TwitterStreamingClient(tweepy.StreamingClient):
    ...
    def on_tweet(self, tweet: tweepy.Tweet):
        try:
            print(json.dumps(tweet.data).encode("utf-8"))
            self.producer.send_data("tweets", tweet.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rules", nargs="+", help="필터링할 rule, hashtag (띄어쓰기로 구분)", default=[])
